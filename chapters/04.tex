...
In section \sectref{sec:4:2}, I will introduce the formal and semiotic concepts that I use to describe the diagrams. In section \sectref{sec:4:3}, I will present early dia\-gramming systems, especially the ones produced by Stephen W. Clark (1810–1901) on the one hand, and Alonzo Reed (d.~1899) and Brainerd Kellogg (1834–1920) on the other hand. Section \sectref{sec:4:4} compares the graphical representation of three specific structures in these two systems.

\section{Visual depictions of syntax}\label{sec:4:2}

Visual depictions of syntactic analyses are diagrams, i.e., icons that represent relations and allow for creative thinking (\citealt{Peirce1994}, cf. \citealt[36--42]{Chauvire2008}). Syntactic diagrams represent words and grammatical concepts, and depict the way they interact. Since words and grammatical concepts are just \textit{thoughts} (albeit highly organized ones), they cannot be expressed without a way of making them perceptible. Syntactic diagrams achieve this goal by graphical means. In section \sectref{sec:4:2.1}, I introduce the concepts of \textit{reification}, \textit{graphical entities} and \textit{spatial configurations}, which I rely on to describe the way linguistic analyses are graphically represented. Section \sectref{sec:4:2.2} illustrates the variety of diagrams. 
...
\begin{figure}
% 	\subfigure[\citet[102]{Billroth1832}]{
	\subfigure[\citeauthor{Billroth1832} (\citeyear{Billroth1832}: 102)]{
	  	\includegraphics[width=.75\textwidth]{figures/04/Billroth.png}
	  	}\\
        \medskip

%         \subfigure[\citet[43]{ReedBrainerd1879}]{
	\subfigure[\citeauthor{ReedBrainerd1879} (\citeyear{ReedBrainerd1879}: 43)]{
...
In the next sections, I will focus on diagrams that share similar rationales with the ones in \figref{fig:4:2}b. In section \sectref{sec:4:3}, I will show how similar rationales are shared by dia\-gramming systems that make different representation choices. In section \sectref{sec:4:4}, I will show that graphical choices constrain what part of the analysis can be expressed, and further force the reification of abstract concepts.

\section{Early syntactic diagrams in the United States: Basic rationales}\label{sec:4:3}

Most dia\-gramming systems from the 19th century reify words, but hardly any relation. Section \sectref{sec:4:3.1} describes the fundamental rationales of the first of these dia\-gramming systems, proposed by Clark in \citeyear{Clark1847}, and compares it with the Reed \& Kellogg system. Section \sectref{sec:4:3.2} briefly illustrates the continuity between the two systems with intermediary diagrams that were proposed by other authors.
...
\begin{figure}
%     \subfigure[\citet[153]{Chandler1862}]{
    \subfigure[\citeauthor{Chandler1862} (\citeyear{Chandler1862}: 153)]{
        \includegraphics[width=0.75\textwidth]{figures/04/Chandler.png}
    }
%     \subfigure[\citet[265]{Burtt1869}]{
    \subfigure[\citeauthor{Burtt1869} (\citeyear{Burtt1869}: 265)]{
        \includegraphics[width=0.75\textwidth]{figures/04/Burtt.png}
    }
%     \subfigure[\citet[51]{Lighthall1872}]{
    \subfigure[\citeauthor{Lighthall1872} (\citeyear{Lighthall1872}: 51)]{
...
Bubbles, open boxes, closed boxes, and strokes are theoretically equivalent. However, strokes are one-dimensional entities, whereas bubbles and boxes are two-dimensional ones. The initial choice of basic entities has consequences for the depiction of more complex constructions. This will be demonstrated in section \sectref{sec:4:4} by comparing Clark’s system to the one developed by Reed and Kellogg.

\section{The logic of space: Case studies}\label{sec:4:4}

In the following subsections, I will study the graphical means at use to represent three kinds of syntactic relations in Clark’s system and in Reed \& Kellogg’s diagrams, namely: the relation between the subject and the predicate (\sectref{sec:4:4.1}), coordination (\sectref{sec:4:4.2}), and subordination of the subject clause (\sectref{sec:4:4.3}). All three relations somewhat break the sparsity of Reed \& Kellogg’s model by introducing additional entities.

\subsection{Subject-predicate relation}\label{sec:4:4.1}

A fundamental relation that every syntactic dia\-gramming system has to depict is the one that holds between the \is{subject}subject and the \is{predicate}predicate. This relation has already been illustrated in §§\ref{sec:4:2.2} and~\ref{sec:4:3.1}.

\begin{figure}
    \subfigure[Clark]{\begin{tikzpicture}%[baseline=(current bounding box.bottom)]
    \draw [rounded corners=12pt] (0,0) rectangle (2,1); 
    \draw [rounded corners=12pt] (4,0) rectangle (2,1);
    \end{tikzpicture}
    }
    \subfigure[Reed \& Kellogg]{
    \begin{tikzpicture}%[baseline=(current bounding box.bottom)]
    \draw [very thick] (0,0) -- (3,0);
    \draw [very thick] (1.5,-.25) -- (1.5,.25);
    \draw [white] (0,0) -- (0,-.5);%add dummy space at bottom
    \end{tikzpicture}
    }
...
\ea \label{ex:4:3} Fulton invented the first steamboat.\z
 ...
 \begin{figure}[t]
     \subfigure[\citeauthor{ReedBrainerd1879} (\citeyear{ReedBrainerd1879}: 53)]{\includegraphics[width=.45\textwidth]{figures/04/ReedKellog2.png}}
     \subfigure[Visual model]{\begin{tikzpicture}%[baseline=(current bounding box.center)]
    \draw [very thick] (0,0) -- (3,0); 
    \draw [very thick] (1,-.25) -- (1,.25);
    \draw [very thick] (2,0) -- (2,.25);
    \draw [white] (0,0) -- (0,-.85); %add dummy space at bottom
    \end{tikzpicture}
    }
...
\begin{figure}[b]
    \subfigure[\citeauthor{ReedBrainerd1879} (\citeyear{ReedBrainerd1879}: 47)]{\includegraphics[width=.65\textwidth]{figures/04/ReedKellog3.png}}
    \subfigure[Visual model]{\begin{tikzpicture}%[baseline=(current bounding box.center)]
                         \draw[thick] (0,0) -- (1,0)
                                      (1,0) -- (2,1)
                                      (2,1) -- (3,1)
...
\is{Subordinate clauses}\text{Subordinate clauses} are not handled in a consistent way, either in Clark’s grammar  (see \citealt[319--322 and 328--329]{Mazziotta2016}) or in the diagrams by Reed and Kellogg. Adjunct clauses are not represented with the same graphical conventions as the ones introduced by complementizers as \is{objects}objects or as \is{subjects}subjects.\footnote{This may be due to the heterogeneous treatment of dependents of the verb, since neither Clark nor Reed \& Kellogg overtly acknowledge verb-centrality (\sectref{sec:4:4.1}).} I will focus on the latter, which Clark names “auxiliary sentences”. Auxiliary clauses assert “dependent proposition[s]” (\citeyear[187]{Clark1870}).

\ea \label{ex:4:4} That good men sometimes commit faults cannot be denied. \z

\begin{figure}
     \subfigure[\citet{Clark1870} (\citeyear{Clark1870}: 47)]{\includegraphics[width=.45\textwidth]{figures/04/Clark3.png}}
     \subfigure[Visual model]{\begin{tikzpicture}%[baseline=(current bounding box.center)]
...
\begin{figure}
     \subfigure[\citeauthor{ReedBrainerd1879} (\citeyear{ReedBrainerd1879}: 81)]{\includegraphics[width=.45\textwidth]{figures/04/ReedKellog4.png}}
     \subfigure[Visual model]{\begin{tikzpicture}%[baseline=(current bounding box.center)]
                      \draw [thick] (0,0) -- (4,0)
                                    (2,-.15) -- (2,.15);
